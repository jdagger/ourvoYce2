...
class BootstrapTopbarList < SimpleNavigation::Renderer::Base
  ...
  def tag_for(item)
    if item.url.nil?
      ...
      if include_sub_navigation?(item) && !options[:is_subnavigation]
        link_to("#{item.name} <b class='caret'></b>", item.url, link_options_for(item))
      else
  ...
  def link_options_for(item)
    ...
    opts['data-toggle'] = data_toggle(item)
    opts.delete('data-toggle') if opts['data-toggle'].nil? || opts['data-toggle'] == ''
    opts
  end

  def data_toggle(item)
    if include_sub_navigation?(item) && !options[:is_subnavigation]
      "dropdown"
    end
  end
